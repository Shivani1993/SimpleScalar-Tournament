...
ident_evaluator(struct eval_state_t *es) /* expression evaluator */
{
	...
	for (i = 0; md_reg_names[i].str != NULL; i++) {
		if (!mystricmp(es->tok_buf, md_reg_names[i].str)) {
			err_str = f_dlite_reg_obj(local_regs, /* !is_write */FALSE,
...
static char *
dlite_help(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_version(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_terminate(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_quit(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_cont(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_step(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_print(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_options(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_option(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_stats(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_stat(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_whatis(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_regs(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_iregs(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_fpregs(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_cregs(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_mstate(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_display(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_dump(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_dis(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_break(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_dbreak(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_rbreak(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_breaks(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_delete(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_clear(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_symbols(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_tsymbols(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_dsymbols(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
static char *
dlite_symbol(int nargs, union arg_val_t args[], struct regs_t *regs,
		struct mem_t *mem);
...
static struct dlite_cmd_t cmd_db[] =
		{ { "help", { "s?", NULL }, dlite_help, "print command reference" }, {
				...
				"print DLite version information" }, { "terminate", { NULL },
				dlite_terminate, "terminate the simulation with statistics" }, {
				"quit", { NULL }, dlite_quit, "exit the simulator" }, { "cont",
				{ "a?", NULL }, dlite_cont,
				"continue program execution (optionally at <addr>)" }, { "step",
				{ NULL }, dlite_step, "step program one instruction" },
#if 0 /* NYI */
				...
				{ "options", { NULL }, dlite_options,
						"print the value of all options" }, { "option", { "s",
						NULL }, dlite_option, "print the value of an option" },
				{ "stats", { NULL }, dlite_stats,
						...
						{ NULL }, NULL, NULL }, { "regs", { NULL }, dlite_regs,
						"print all register contents" }, { "iregs", { NULL },
						dlite_iregs, "print integer register contents" }, {
						"fpregs", { NULL }, dlite_fpregs,
						"print floating point register contents" },
				{ "cregs", { NULL }, dlite_cregs,
						"print control register contents" }, { "mstate", { "s?",
						NULL }, dlite_mstate,
				...
				{ "breaks", { NULL }, dlite_breaks,
						"list active code and data breakpoints" }, { "delete", {
						"i", NULL }, dlite_delete, "delete breakpoint <id>" }, {
				...
				{ "symbols", { NULL }, dlite_symbols,
						...
						"print the value of a symbol" }, { "---", { NULL },
						NULL, NULL },
				/* list terminator */
				{ NULL, { NULL }, NULL, NULL } };
...
struct mem_t *mem) /* memory to access */
{
	...
	*q = '\0';
	...
	for (i = 0, arg_cnt = 0; i < MAX_ARGS && cmd->arg_strs[i] != NULL;
			i++, arg_cnt++) {
...
struct eval_value_t val) /* expr value to print */
{
	...
	if (modifiers & (MOD_BYTE | MOD_HALF | MOD_WORD | MOD_QWORD)) {
		...
		/* no format specified, default to value type format */
...
static void print_help(struct dlite_cmd_t *cmd) /* command to describe */
{
	int i;
...
struct mem_t *mem) /* memory to access */
{
	...
	if (modifiers & (MOD_BYTE | MOD_CHAR))
		...
		/* no modifiers, or MOD_WORD|MOD_FLOAT */
		size = 4;

	/* read memory */
	errstr = f_dlite_mem_obj(mem, /* !is_write */FALSE, addr, (char *) buf,
			size);
	if (errstr)
		return errstr;
...
{
	int i, j;
	int count = LINES_PER_SCREEN * BYTES_PER_LINE, i_count, fmt_count,
			fmt_lines;
	...
	if (fmt_lines == 1) {
		...
		for (i = 0; i < count; i++) {
			errstr = f_dlite_mem_obj(mem, /* !is_write */FALSE, i_addr,
					(char *) &byte, 1);
			if (errstr)
				return errstr;
...
struct mem_t *mem) /* memory to access */
{
	...
	for (i = 0; i < count; i++) {
		...
		errstr = f_dlite_mem_obj(mem, /* !is_write */FALSE, addr,
				(char *) &inst, sizeof(inst));
		inst = MD_SWAPI(inst);
		if (errstr)
			return errstr;
...
void dlite_init(dlite_reg_obj_t reg_obj, /* register state object */
...
static void dlite_status(md_addr_t regs_PC, /* PC of just completed inst */
...
struct mem_t *mem) /* memory to access */
{
	...
	if (dbreak) {
		...
		errstr = f_dlite_mem_obj(mem, /* !is_write */FALSE, regs_PC,
				(char *) &inst, sizeof(inst));
		inst = MD_SWAPI(inst);
		if (errstr)
	...
	errstr = f_dlite_mem_obj(mem, /* !is_write */FALSE, next_PC, (char *) &inst,
			sizeof(inst));
	inst = MD_SWAPI(inst);
	if (errstr)
		fprintf(stdout, "<invalid memory>: %s", errstr);
