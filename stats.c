...
struct eval_value_t stat_eval_ident(struct eval_state_t *es)/* an expression evaluator */
{
...
void stat_delete(struct stat_sdb_t *sdb) /* stats database */
{
	...
	for (stat = sdb->stats; stat != NULL; stat = stat_next) {
		...
		case sc_sdist:
			...
			for (i = 0; i < HTAB_SZ; i++) {
				for (bucket = stat->variant.for_sdist.sarr[i]; bucket != NULL;
						bucket = bucket_next) {
...
int nsamples) /* number of samples to add to dist */
{
	...
	case sc_sdist: {
		...
		/* find bucket */
		for (bucket = stat->variant.for_sdist.sarr[hash]; bucket != NULL;
				bucket = bucket->next) {
...
static int compare_fn(void *p1, void *p2) {
...
static void print_dist(struct stat_stat_t *stat, /* stat variable */
FILE *fd) /* output stream */
{
	...
	fprintf(fd, "%s.array_size = %u\n", stat->name,
			stat->variant.for_dist.arr_sz);
	fprintf(fd, "%s.bucket_size = %u\n", stat->name,
			stat->variant.for_dist.bucket_sz);
	...
	fprintf(fd, "%s.overflows = %u\n", stat->name,
			stat->variant.for_dist.overflows);
	...
	if (bcount > 0) {
		...
		for (i = 0; i < bcount; i++) {
			...
			if (stat->variant.for_dist.print_fn) {
				stat->variant.for_dist.print_fn(stat, i,
						stat->variant.for_dist.arr[i], bsum, btotal);
			} else {
				if (stat->format == NULL) {
...
static void print_sdist(struct stat_stat_t *stat, /* stat variable */
FILE *fd) /* output stream */
{
	...
	imin = UINT_MAX;
	for (i = 0; i < HTAB_SZ; i++) {
		for (bucket = stat->variant.for_sdist.sarr[i]; bucket != NULL; bucket =
				bucket->next) {
			bcount++;
			btotal += bucket->count;
			/* on-line variance computation, tres cool, no!?! */
	...
	if (bcount > 0) {
		...
		for (i = 0; i < bcount; i++) {
			...
			if (stat->variant.for_sdist.print_fn) {
				stat->variant.for_sdist.print_fn(stat, barr[i]->index,
						barr[i]->count, bsum, btotal);
			} else {
				if (stat->format == NULL) {
					...
					if (pf & PF_PDF)
						fprintf(fd, "%6.2f ",
								(double) barr[i]->count / MAX(btotal, 1.0)
										* 100.0);
					if (pf & PF_CDF)
						fprintf(fd, "%6.2f ", bsum / MAX(btotal, 1.0) * 100.0);
				} else {
					if (pf == (PF_COUNT | PF_PDF | PF_CDF)) {
						myfprintf(fd, stat->format, barr[i]->index,
								barr[i]->count,
								(double) barr[i]->count / MAX(btotal, 1.0)
										* 100.0,
								bsum / MAX(btotal, 1.0) * 100.0);
