...
eio_open(char *fname) {
	...
	exo = exo_read(fd);
	if (!exo || exo->ec != ec_list || !exo->as_list.head
...
FILE *fd) /* stream to write to */
{
	...
	for (i = 0; i < MD_NUM_IREGS; i++)
		exo->as_list.head = exo_chain(exo->as_list.head,
				MD_IREG_TO_EXO(regs, i));
	exo_print(exo, fd);
	fprintf(fd, "\n\n");
	exo_delete(exo);
	...
	for (i = 0; i < MD_NUM_FREGS; i++)
		exo->as_list.head = exo_chain(exo->as_list.head,
				MD_FREG_TO_EXO(regs, i));
	exo_print(exo, fd);
	fprintf(fd, "\n\n");
	exo_delete(exo);

	fprintf(fd, "/* writing `%d' memory pages... */\n", (int) mem->page_count);
	exo = exo_new(ec_list, exo_new(ec_integer, (exo_integer_t) mem->page_count),
	...
	exo = exo_new(ec_list, exo_new(ec_address, (exo_integer_t) ld_text_base),
	...
	exo = exo_new(ec_list, exo_new(ec_address, (exo_integer_t) ld_data_base),
	...
	exo = exo_new(ec_list, exo_new(ec_address, (exo_integer_t) ld_stack_base),
...
static enum md_fault_type my_mem_fn(struct mem_t *mem, /* memory space to access */
...
int nbytes) /* number of bytes to access */
{
	...
	if (mem_rec->exo != NULL && (mem_rec->addr + mem_rec->size == addr)
		...
		/* add to a new blob */
		mem_list->as_list.head = exo_chain(mem_list->as_list.head,
				(mem_rec->exo = exo_new(ec_list,
						exo_new(ec_address, (exo_integer_t) addr),
						(mem_rec->blob = exo_new(ec_blob,
								nbytes + BLOB_TAIL_SIZE, NULL)),
						NULL)));
		for (i = 0; i < nbytes; i++)
...
md_inst_t inst) /* system call inst */
{
	...
	for (i = MD_FIRST_IN_REG; i <= MD_LAST_IN_REG; i++) {
		input_regs->as_list.head = exo_chain(input_regs->as_list.head,
				MD_IREG_TO_EXO(regs, i));
	}
	...
	output_regs->as_list.head = exo_chain(output_regs->as_list.head,
			exo_new(ec_address, (exo_integer_t) ld_brk_point));
	...
	/* PC */exo_new(ec_address, (exo_integer_t) regs->regs_PC), input_regs,
			input_mem, output_regs, output_mem,
			NULL);
...
md_inst_t inst) /* system call inst */
{
	...
	/* check memory inputs */
	for (memrec = exo_inmem->as_list.head; memrec != NULL; memrec =
			memrec->next) {
		md_addr_t loc;
		...
		for (loc = addr->as_integer.val, i = 0; i < blob->as_blob.size;
			...
			if (val != blob->as_blob.data[i])
				fatal("EIO trace inconsistency: addr 0x%08p input mismatch",
						loc);
		...
		if (MD_OUTPUT_SYSCALL(regs)) {
			if (sim_progfd) {
				...
				write(MD_STREAM_FILENO(regs), blob->as_blob.data,
						blob->as_blob.size);
			}
		}
	}
	...
	for (memrec = exo_outmem->as_list.head; memrec != NULL;
		...
		for (loc = addr->as_integer.val, i = 0; i < blob->as_blob.size;
			...
			(*mem_fn)(mem, Write, loc, &blob->as_blob.data[i],
