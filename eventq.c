...
EVENTQ_ID_TYPE eventq_queue_setbit(SS_TIME_TYPE when, BITMAP_ENT_TYPE *bmap,
		int sz, int bitnum) {
	EVENTQ_ID_TYPE id;
	__QUEUE_EVENT(when, id,
			new->action = EventSetBit; new->data.bit.bmap = bmap; new->data.bit.sz = sz; new->data.bit.bitnum = bitnum);
	return id;
}
...
EVENTQ_ID_TYPE eventq_queue_clearbit(SS_TIME_TYPE when, BITMAP_ENT_TYPE *bmap,
	...
	EVENTQ_ID_TYPE id;
	__QUEUE_EVENT(when, id,
			new->action = EventClearBit; new->data.bit.bmap = bmap; new->data.bit.sz = sz; new->data.bit.bitnum = bitnum);
	return id;
}

EVENTQ_ID_TYPE eventq_queue_setflag(SS_TIME_TYPE when, int *pflag, int value) {
	EVENTQ_ID_TYPE id;
	__QUEUE_EVENT(when, id,
			new->action = EventSetFlag; new->data.flag.pflag = pflag; new->data.flag.value = value);
	return id;
}

EVENTQ_ID_TYPE eventq_queue_addop(SS_TIME_TYPE when, int *summand, int addend) {
	EVENTQ_ID_TYPE id;
	__QUEUE_EVENT(when, id,
			new->action = EventAddOp; new->data.addop.summand = summand; new->data.addop.addend = addend);
	return id;
}
...
EVENTQ_ID_TYPE eventq_queue_callback(SS_TIME_TYPE when,
	...
	EVENTQ_ID_TYPE id;
	__QUEUE_EVENT(when, id,
			new->action = EventCallback; new->data.callback.fn = fn; new->data.callback.arg = arg);
	return id;
}
...
void eventq_dump(FILE *stream) {
	...
	for (ev = eventq_pending; ev; ev = ev->next) {
		fprintf(stream, "@ %.0f:%s:", (double) ev->when,
				...
				ev->action == EventCallback ? "call back" : (abort(), ""));
		switch (ev->action) {
		case EventSetBit:
		case EventClearBit:
			fprintf(stream, "0x%p, %d, %d", ev->data.bit.bmap, ev->data.bit.sz,
					ev->data.bit.bitnum);
			break;
		case EventSetFlag:
			fprintf(stream, "0x%p, %d", ev->data.flag.pflag,
					ev->data.flag.value);
			break;
		case EventAddOp:
			fprintf(stream, "0x%p, %d", ev->data.addop.summand,
					ev->data.addop.addend);
			break;
		case EventCallback:
			fprintf(stream, "0x%p, %d", ev->data.callback.fn,
					ev->data.callback.arg);
			break;
		default:
			panic("bogus event action");
		}
