...
void sim_reg_options(struct opt_odb_t *odb) {
	...
	opt_reg_flag(odb, "-all", "enable all profile options", &prof_all, /* default */
			FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-iclass", "enable instruction class profiling", &prof_ic, /* default */
			FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-iprof", "enable instruction profiling", &prof_inst, /* default */
			FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-brprof", "enable branch instruction profiling",
			&prof_bc, /* default */FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-amprof", "enable address mode profiling", &prof_am, /* default */
			FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-segprof", "enable load/store address segment profiling",
			&prof_seg, /* default */FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-tsymprof", "enable text symbol profiling", &prof_tsyms, /* default */
			FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-taddrprof", "enable text address profiling",
			&prof_taddr, /* default */FALSE, /* print */TRUE, NULL);

	opt_reg_flag(odb, "-dsymprof", "enable data symbol profiling", &prof_dsyms, /* default */
			FALSE, /* print */TRUE, NULL);
...
static char *inst_class_str[ic_NUM] = { "load", /* load inst */
"store", /* store inst */
...
static char *branch_class_str[bc_NUM] = { "uncond direct", /* direct unconditional branch */
...
static char *addr_seg_str[seg_NUM] = { "data segment", /* data segment */
...
bind_to_seg(md_addr_t addr) /* address to bind to a segment */
{
	if (ld_data_base <= addr && addr < (ld_data_base + ld_data_size))
		...
		return seg_heap;
	/* FIXME: ouch! */
	else if ((ld_stack_base - (16 * 1024 * 1024)) <= addr
			&& addr < ld_stack_base)
		return seg_stack;
...
void sim_reg_stats(struct stat_sdb_t *sdb) {
	...
	if (prof_inst) {
		...
		/* instruction profile */
		inst_prof = stat_reg_dist(sdb, "sim_inst_prof", "instruction profile",
		/* initial value */0,
		...
		if (prof_bc) {
			...
			if (flags & F_CTRL) {
				if ((flags & (F_CALL | F_DIRJMP)) == (F_CALL | F_DIRJMP))
					bc = bc_call_dir;
				else if ((flags & (F_CALL | F_INDIRJMP))
						== (F_CALL | F_INDIRJMP))
					bc = bc_call_indir;
				else if ((flags & (F_UNCOND | F_DIRJMP))
						== (F_UNCOND | F_DIRJMP))
					bc = bc_uncond_dir;
				else if ((flags & (F_UNCOND | F_INDIRJMP))
						== (F_UNCOND | F_INDIRJMP))
					bc = bc_uncond_indir;
				else if ((flags & (F_COND | F_DIRJMP)) == (F_COND | F_DIRJMP))
					bc = bc_cond_dir;
				else if ((flags & (F_COND | F_INDIRJMP))
						== (F_COND | F_INDIRJMP))
					bc = bc_cond_indir;
				else
