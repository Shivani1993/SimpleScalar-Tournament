...
void fatal_hook(void (*fn)(FILE *stream)) /* fatal hook function */
{
	hook_fn = fn;
}
...
#ifdef __GNUC__
void _fatal(char *file, const char *func, int line, char *fmt, ...)
#else /* !__GNUC__ */
void
fatal(char *fmt, ...)
#endif /* __GNUC__ */
...
#ifdef __GNUC__
void _panic(char *file, const char *func, int line, char *fmt, ...)
#else /* !__GNUC__ */
void
panic(char *fmt, ...)
#endif /* __GNUC__ */
...
static int _lowdigit(slargeint_t *valptr) {
...
#define MAXDIGS		22
	...
	for (;;) {
		...
		charswitch: switch (fcode = *format++) {
