...
static enum {
	no_chkpt, one_shot_chkpt, periodic_chkpt
} chkpt_kind = no_chkpt;
...
void sim_reg_options(struct opt_odb_t *odb) {
	...
	opt_reg_int(odb, "-fastfwd",
			"number of insts skipped before tracing starts", &fastfwd_count, /* default */
			0,
			/* print */TRUE, /* format */NULL);
	...
	opt_reg_string_list(odb, "-perdump",
			"periodic checkpoint every n instructions: "
					"<base fname> <interval>", per_chkpt_opts, /* sz */2,
			&per_chkpt_nelt,
			/* default */NULL,
			/* !print */FALSE, /* format */NULL, /* !accrue */FALSE);

	opt_reg_string_list(odb, "-dump",
			"specify checkpoint file and trigger: <fname> <range>", chkpt_opts, /* sz */
			2, &chkpt_nelt, /* default */NULL,
			/* !print */FALSE, /* format */NULL, /* !accrue */FALSE);
...
char **envp) /* program environment */
{
	...
	if (chkpt_nelt == 2) {
		...
		if (!sim_eio_fd)
			fatal(
					"checkpoints can only be generated while running an EIO trace");
...
#include "machine.def"
	...
	while (TRUE) {
		...
		if (chkpt_kind == one_shot_chkpt
				&& !range_cmp_range1(&chkpt_range, regs.regs_NPC, sim_num_insn,
						sim_num_insn)) {
			myfprintf(stderr,
