...
unsigned int retstack_size) /* num entries in ret-addr stack */
{
	...
	case BPredTournament:
		// global predictor
		pred->dirpred.twolev = bpred_dir_create(BPred2Level, 1, l2size, shift_width, xor);
		// local predictor
		pred->dirpred.twolev2 = bpred_dir_create(BPred2Level, l1size, l2size, shift_width, xor);
		break;
...
*/
	...
	if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
			(pred->class == BPred2Level || pred->class == BPredComb || pred->class == BPredTournament))
	{
