...
unsigned int retstack_size) /* num entries in ret-addr stack */
{
	...
	case BPredTournament:
		// global predictor
		pred->dirpred.twolev = bpred_dir_create(BPred2Level, 1, l2size,
				shift_width, xor);
		// local predictor
	...
	case BPredComb:
		...
		/* 2-level component */
		pred->dirpred.twolev = bpred_dir_create(BPred2Level, l1size, l2size,
				shift_width, xor);
...
unsigned int xor) /* history xor address flag */
{
	...
	case BPred2bit:
		...
		if (!(pred_dir->config.bimod.table = calloc(l1size,
...
FILE *stream) /* output stream */
{
	...
	case BPredTournament:
		fprintf(stream,
				...
				name, pred_dir->config.two.l1size,
...
FILE *stream) /* output stream */
{
	...
	case BPredTournament:
		...
		bpred_dir_config(pred->dirpred.twolev2, "2lev2", stream);
		fprintf(stream, "btb: %d sets x %d associativity", pred->btb.sets,
				pred->btb.assoc);
...
FILE *stream) /* output stream */
{
	fprintf(stream, "pred: addr-prediction rate = %f\n",
			(double) pred->addr_hits
					/ (double) (pred->addr_hits + pred->misses));
...
struct stat_sdb_t *sdb) /* stats database */
{
	...
	stat_reg_counter(sdb, buf, "total number of bpred lookups", &pred->lookups,
			0, NULL);
	...
	stat_reg_counter(sdb, buf,
			"total number of address-predicted hits for JR's", &pred->jr_hits,
			0, NULL);
	sprintf(buf, "%s.jr_seen", name);
	stat_reg_counter(sdb, buf, "total number of JR's seen", &pred->jr_seen, 0,
			NULL);
	...
	stat_reg_counter(sdb, buf, "total number of non-RAS JR's seen",
	...
	stat_reg_counter(sdb, buf, "total number of RAS hits", &pred->ras_hits, 0,
			NULL);
 	...
 	/* used on mispredict recovery */
{
	...
	if (pbtb == NULL) {
		...
		return ((*(dir_update_ptr->pdir1) >= 2) ? /* taken */
				1 : /* not taken */0);
	} else {
		/* BTB hit, so return target if it's a predicted-taken branch */
		return ((*(dir_update_ptr->pdir1) >= 2) ? /* taken */
				pbtb->target : /* not taken */0);
	}
}
