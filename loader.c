...
void ld_reg_stats(struct stat_sdb_t *sdb) /* stats data base */
{
	stat_reg_addr(sdb, "ld_text_base", "program text (code) segment base",
			&ld_text_base, ld_text_base, "0x%010p");
	stat_reg_uint(sdb, "ld_text_size", "program text (code) size in bytes",
			&ld_text_size, ld_text_size, NULL);
	stat_reg_addr(sdb, "ld_data_base", "program initialized data segment base",
...
#endif
	stat_reg_uint(sdb, "ld_stack_size", "program initial stack size",
			&ld_stack_size, ld_stack_size, NULL);
	stat_reg_addr(sdb, "ld_prog_entry", "program entry point (initial PC)",
			&ld_prog_entry, ld_prog_entry, "0x%010p");
	stat_reg_addr(sdb, "ld_environ_base",
			"program environment base address address", &ld_environ_base,
			ld_environ_base, "0x%010p");
...
int zero_bss_segs) /* zero uninit data segment? */
{
...

	{
		...
		for (i = 0; i < MD_SWAPH(fhdr.f_nscns); i++) {
			...
			case ECOFF_STYP_TEXT:
				...
				mem_bcopy(mem_access, mem, Write, MD_SWAPQ(shdr.s_vaddr), p,
						MD_SWAPQ(shdr.s_size));
	...
	for (i = 0; i < argc; i++) {
		...
		temp = MD_SWAPQ(sp);
		mem_access(mem, Write, argv_addr + i * sizeof(md_addr_t), &temp,
				sizeof(md_addr_t));
		/* and the data */
	...
	mem_access(mem, Write, argv_addr + i * sizeof(md_addr_t), &null_ptr,
			sizeof(md_addr_t));
	...
	for (i = 0; envp[i]; i++) {
		...
		temp = MD_SWAPQ(sp);
		mem_access(mem, Write, envp_addr + i * sizeof(md_addr_t), &temp,
				sizeof(md_addr_t));
		/* and the data */
	...
	mem_access(mem, Write, envp_addr + i * sizeof(md_addr_t), &null_ptr,
			sizeof(md_addr_t));
	...
	debug("ld_text_base: 0x%08x  ld_text_size: 0x%08x",
			ld_text_base, ld_text_size); debug("ld_data_base: 0x%08x  ld_data_size: 0x%08x",
			ld_data_base, ld_data_size); debug("ld_stack_base: 0x%08x  ld_stack_size: 0x%08x",
			ld_stack_base, ld_stack_size); debug("ld_prog_entry: 0x%08x", ld_prog_entry);
}
