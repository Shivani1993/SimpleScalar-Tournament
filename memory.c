...
md_addr_t addr) /* virtual address to translate */
{
	...
	/* locate accessed PTE */
	for (prev = NULL, pte = mem->ptab[MEM_PTAB_SET(addr)]; pte != NULL; prev =
			pte, pte = pte->next) {
...
int nbytes) /* number of bytes to access */
{
	...
	/* check alignments */
	if (/* check size */(nbytes & (nbytes - 1))
			!= 0 || /* check max size */nbytes > MD_PAGE_SIZE)
...
struct stat_sdb_t *sdb) /* stats data base */
{
	...
	stat_reg_formula(sdb, buf, "total size of memory pages allocated", buf1,
			"%11.0fk");
	...
	stat_reg_counter(sdb, buf, "total page table accesses", &mem->ptab_accesses,
			mem->ptab_accesses, NULL);
